...
class GetProduct(Request):
	def __init__(self, ProductNumber, key=""):
		Request.__init__(self, key)
		if key == '':
			raise TypeError("An API key was not provided (not enough aruments for " +
							self.__class__.__name__ + " request)")
		self.el_productgetrq = etree.SubElement(self.el_fbimsgsrq, 'ProductGetRq')
		self.el_number = etree.SubElement(self.el_productgetrq, 'Number')
		self.el_number.text = ProductNumber
		self.el_getimage = etree.SubElement(self.el_productgetrq, 'GetImage')
		self.el_getimage.text = str(True)
		xmlmsg = etree.tostring(self.el_fbixml, pretty_print=True)
		self.request = xmlmsg

class GetPart(Request):
	def __init__(self, PartNumber, key=""):
		Request.__init__(self, key)
		if key == '':
			raise TypeError("An API key was not provided (not enough aruments for " +
							self.__class__.__name__ + " request)")
		self.el_productgetrq = etree.SubElement(self.el_fbimsgsrq, 'PartGetRq')
		self.el_number = etree.SubElement(self.el_productgetrq, 'Number')
		self.el_number.text = PartNumber
		self.el_getimage = etree.SubElement(self.el_productgetrq, 'GetImage')
		self.el_getimage.text = str(True)
		xmlmsg = etree.tostring(self.el_fbixml, pretty_print=True)
		self.request = xmlmsg